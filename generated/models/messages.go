...
var MessageWhere = struct {
	...
	MessageID: whereHelperint{field: "`messages`.`message_id`"},
	SpaceID:   whereHelperint{field: "`messages`.`space_id`"},
	Author:    whereHelperstring{field: "`messages`.`author`"},
...
func (r *messageR) GetSpace() *Space {
	if r == nil {
		return nil
	}
	return r.Space
...
var (
	messageAllColumns            = []string{"message_id", "space_id", "author", "content", "timestamp"}
	messageColumnsWithoutDefault = []string{"message_id", "space_id", "author", "content"}
...
func (o *Message) Space(mods ...qm.QueryMod) spaceQuery {
	queryMods := []qm.QueryMod{
		qm.Where("`space_id` = ?", o.SpaceID),
	}

	queryMods = append(queryMods, mods...)

	return Spaces(queryMods...)
}

// LoadSpace allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (messageL) LoadSpace(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMessage interface{}, mods queries.Applicator) error {
	var slice []*Message
	var object *Message
	...
	query := NewQuery(
		qm.From(`spaces`),
		qm.WhereIn(`spaces.space_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// SetSpace of the message to the related item.
// Sets o.R.Space to related.
// Adds o to related.R.Messages.
func (o *Message) SetSpace(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Space) error {
	var err error
	if insert {
	...
	values := []interface{}{related.SpaceID, o.MessageID}
...
func FindMessage(ctx context.Context, exec boil.ContextExecutor, messageID int, selectCols ...string) (*Message, error) {
	messageObj := &Message{}
	...
	query := fmt.Sprintf(
		"select %s from `messages` where `message_id`=?", sel,
	)
...
func (o *Message) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for messages")
	}
...
func (o *Message) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for messages")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for messages")
	}
...
func MessageExists(ctx context.Context, exec boil.ContextExecutor, messageID int) (bool, error) {
	var exists bool
